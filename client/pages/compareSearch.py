...
with col2:
  ...
  options = st.multiselect('Types of restaurants', types)
  radius = st.slider('Radius', min_value=1000, max_value=100000, value=1000)
  writeR('<br><br>')
  if st.button("Search", key='search') and name and address and options:
    switch_page('selectPlace')
  elif st.session_state['search'] and not (name and address and options):
    writeR('<br><br>')
